...
`packages` parameter _must_ be a function that receives the imported `nixpkgs`
...
`nix/pre-commit.nix` file _and_ include a [`pre-commit-hooks`
input](https://github.com/cachix/pre-commit-hooks.nix) in your flake.
...
To run the tests, make sure to include the [nixt
input](https://github.com/nix-community/nixt) in your flake.

``` bash
nix run .#nixt
```

You may disable the generation of the nixt application by setting the

...
let
  ...
  getDevenvModule = moduleName:
    if builtins.typeOf moduleName == "string" then
      ...
      throw ''
        nixDir is confused, it was expecting an entry in the `injectDevenvModules` list to contain devenv module names.

        Available modules are: ${injectDevenvModules}
      '';

  ...
  mkBuildFlakeCfg =
    ...
    let
      ...
      getEntriesForPath = path:
        if lib.hasSuffix "nix/devShells" path then
          devShellNames
        else
          [ ];
    ...
    # configuration value used in the nixDir.lib.buildFlake call
    {
      ...
      inherit importDevShells pathExists getEntriesForPath;
    };
          ...
          (it spec.it
            (
              let
                ...
                sut = import "${self}/nix/src/devshells.nix" nixDirInputs buildFlakeCfg;
